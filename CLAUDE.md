...
- **Behavior** (`CourtList.test.tsx`) — renders with mock court data and asserts the Nearby/Saved tab counts, `onSelect` on click, scroll-into-view on selection change, conditional rating / open-closed rendering, the directions link, the star toggle (calls `onToggleFavorite`, doesn't select the row, reflects `aria-pressed`), the Saved tab (lists favorites, empty state), and the amenity filter (segmented control → `onFilterChange`, the "N hidden — Show all" note, the empty-filtered state, and the tab showing the full `nearbyTotal` while the list is filtered down).
...
- **Hook** (`usePickleballMap.test.tsx`) — drives the hook through a `Harness` component against a faked `google.maps` global (the fake `AdvancedMarkerElement` exposes `addEventListener`, matching the real HTMLElement, and registers each instance so tests can count "live" pins). Captures the hook return in an effect (not during render) to satisfy `react-hooks` lint rules. Covers geocode→search→Court mapping, the keep-prior-results re-search, and the amenity filter: `displayedCourts` narrows while `courts` stays whole, the live marker count tracks the displayed list in lockstep (the pin↔list regression the plan guards against), and the filter resets on a fresh search.
